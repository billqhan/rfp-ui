...
if [ -z "$BUCKET_NAME" ]; then
    echo -e "${RED}❌ Bucket name is empty. Provide bucket arg or set UI_BUCKET or BUCKET_PREFIX/ENVIRONMENT variables.${NC}"
...
echo ""
echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║          WEB UI DEPLOYMENT SCRIPT                     ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
CF_STACK_NAME="rfp-han-${ENVIRONMENT}-cloudfront-ui"
CF_EXISTS=$(aws cloudformation describe-stacks \
    --stack-name "$CF_STACK_NAME" \
    --region "$REGION" \
    --query 'Stacks[0].StackStatus' \
    --output text 2>/dev/null || echo "")

if [ -n "$CF_EXISTS" ] && [ "$CF_EXISTS" != "DELETE_COMPLETE" ]; then
    echo -e "${GREEN}  ✅ CloudFront distribution found${NC}"
    
    # Use relative path - CloudFront proxies /api/* to ALB
    cat > "$SCRIPT_DIR/$ENV_FILE" <<EOF
# ${ENVIRONMENT} environment configuration
# Using relative path - CloudFront proxies /api/* to ALB
VITE_API_BASE_URL=/api
VITE_APP_ENV=${ENVIRONMENT}
VITE_DEBUG=false
EOF
    echo -e "${GREEN}  ✅ Configured for CloudFront with relative API path${NC}"
else
    # Fallback to direct ALB access
    ...
    if [ -n "$ALB_URL" ] && [ "$ALB_URL" != "None" ]; then
        echo -e "${GREEN}  ✅ Found ALB endpoint: $ALB_URL${NC}"
        
        # Create/update environment-specific file with ALB endpoint
        cat > "$SCRIPT_DIR/$ENV_FILE" <<EOF
# ${ENVIRONMENT} environment configuration
...
VITE_DEBUG=false
EOF
        echo -e "${GREEN}  ✅ Updated $ENV_FILE with ALB endpoint${NC}"
    else
        echo -e "${YELLOW}  ⚠️  ALB stack not found, checking for existing $ENV_FILE...${NC}"
        if [ -f "$SCRIPT_DIR/$ENV_FILE" ]; then
            echo -e "${GREEN}  ✅ Using existing $ENV_FILE${NC}"
        else
            echo -e "${RED}  ❌ No $ENV_FILE file and ALB stack not found${NC}"
            echo -e "${YELLOW}  ℹ️  Troubleshooting:${NC}"
            echo "     1. Deploy ALB stack first:"
            echo "        cd ../rfp-infrastructure && ./scripts/deploy-alb.sh"
            echo "     2. Or manually create $ENV_FILE with API endpoint:"
...
echo -e "${BLUE}  📤 Uploading $FILE_COUNT files to s3://$BUCKET_NAME${NC}"
...
if [ -n "$CLOUDFRONT_ID" ] && [ "$CLOUDFRONT_ID" != "None" ]; then
    echo -e "${BLUE}  🔄 Invalidating CloudFront cache for distribution: $CLOUDFRONT_ID${NC}"
...
if [ -n "$CLOUDFRONT_URL" ]; then
    ...
    echo -e "${BLUE}⏱️  Cache Invalidation:${NC} $INVALIDATION_ID"
    echo -e "${YELLOW}ℹ️  Changes will be visible after cache invalidation completes (1-3 minutes)${NC}"
else
    echo -e "${BLUE}📦 S3 Location:${NC} s3://$BUCKET_NAME"
    echo -e "${YELLOW}ℹ️  Access via CloudFront distribution (check CloudFormation outputs)${NC}"
fi

if [ -n "$ALB_URL" ]; then
    echo -e "${BLUE}🔗 API Endpoint:${NC} $ALB_URL/api"
fi

echo ""
echo -e "${YELLOW}📋 Post-Deployment Verification:${NC}"
echo "   1. Test CloudFront URL in browser (wait 1-3 minutes for cache)"
echo "   2. Check browser console for API errors (F12 → Console)"
